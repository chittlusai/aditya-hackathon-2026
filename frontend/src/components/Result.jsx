  ...
  if (!result) {
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center space-y-4">
        ...
        <button
          onClick={() => go('check')}
          className="tap-press inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#6D8196] hover:bg-[#56687B] text-[#FFFFE3] font-bold text-xs shadow-sm"
        >
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-5">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
        <button
          onClick={() => go('check')}
          className="tap-press inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 text-xs font-bold shadow-sm"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="tap-press inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-sm"
          >
      ...
      {isEmergency && (
        <div className="p-4 rounded-2xl bg-red-50 border border-red-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600">{t('emergencyBannerSub')}</p>
            </div>
          </div>
          ...
          <button
            ...
            className="tap-press px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-bold shadow-sm shrink-0"
          >
            {t('call108Ambulance')}
          </button>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        {result.first_aid && (
          <div className="p-4 bg-amber-50/60 border border-amber-200 rounded-xl text-xs text-slate-800 space-y-1.5">
            ...
            <p className="leading-relaxed text-slate-700">{result.first_aid}</p>
          </div>
        )}
          ...
          <div className="pt-2">
            ...
            <div className="flex flex-wrap gap-2">
              {result.risk_factors.map((rf, i) => (
                <span
                  key={i}
                  className="text-xs px-3 py-1 rounded-xl bg-red-50 text-red-900 border border-red-200 font-semibold flex items-center gap-1.5"
                >
        ...
        {result.matched_keywords && result.matched_keywords.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {result.matched_keywords.map((kw, i) => (
              <span
                key={i}
                className="text-xs px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 border border-slate-200 font-semibold flex items-center gap-1"
              >
      ...
      <div>
        ...
        {result.hospital ? (
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 text-slate-700 text-xs">
            {t('mildSub')}
          </div>
        )}
      </div>
