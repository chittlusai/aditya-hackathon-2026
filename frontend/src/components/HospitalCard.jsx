  ...
  return (
    <div
      className={`bg-white rounded-xl overflow-hidden border shadow-sm ${
        isTop ? 'border-blue-700 ring-1 ring-blue-700' : 'border-slate-300'
      }`}
    >
      {/* Top Header Strip */}
      <div className="bg-slate-100 px-5 py-3 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className={`text-[11px] font-bold px-2 py-0.5 rounded border ${
            isTop ? 'bg-blue-800 text-white border-blue-900' : 'bg-slate-200 text-slate-800 border-slate-300'
          }`}>
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          {hospital.phone && (
            <a
              ...
              className="tap-press inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-950 border border-blue-200 font-bold text-xs shadow-sm"
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          ...
          <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200">
            ...
            <p className="font-bold text-xs text-emerald-800">{hospital.doctors_available} {t('inStock')}</p>
          </div>
          ...
          <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200">
            ...
            <p className="font-bold text-xs text-slate-800 truncate">{hospital.specialist?.split(',')[0] || 'General'}</p>
          </div>
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-center gap-2.5">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="tap-press w-full sm:flex-1 min-h-[40px] rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold flex items-center justify-center gap-1.5 border border-slate-300"
          >
          ...
          {hospital.phone && (
            <a
              href={`tel:${hospital.phone}`}
              className="tap-press w-full sm:flex-1 min-h-[40px] rounded-lg bg-blue-800 hover:bg-blue-900 text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm"
            >
              <Phone className="w-3.5 h-3.5" />
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="mt-3 w-full flex items-center justify-between p-2.5 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold tap-press"
          aria-expanded={open}
        >
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              <div className="mt-2.5 p-3 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-700 space-y-1.5 leading-relaxed">
