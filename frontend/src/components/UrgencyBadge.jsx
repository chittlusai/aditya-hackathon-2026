...
export default function UrgencyBadge({ urgency }) {
  ...
  const tierConfig = {
    ...
    bg: 'bg-primary-50 border-primary-200 text-primary-900',
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-white border border-border-soft rounded-lg p-3 sm:p-4 shadow-sm">
        ...
        <div className="grid grid-cols-3 gap-2">
          {/* Level 1: Mild */}
          <div
            className={`p-2 sm:p-2.5 rounded-md border text-center transition-all ${
              isMild
                ? 'bg-emerald-600 text-white border-emerald-600 font-extrabold shadow-sm ring-2 ring-emerald-500/20'
                : 'bg-slate-50 text-slate-400 border-border-soft opacity-60'
            }`}
          >
          ...
          <div
            className={`p-2 sm:p-2.5 rounded-md border text-center transition-all ${
              isEm
                ? 'bg-red-600 text-white border-red-600 font-extrabold shadow-sm ring-2 ring-red-500/20'
                : 'bg-slate-50 text-slate-400 border-border-soft opacity-60'
            }`}
          >
            <div className="flex items-center justify-center gap-1 text-[11px] sm:text-xs">
      ...
      <div className={`p-4 sm:p-5 rounded-lg border ${tierConfig.bg} ${tierConfig.borderRing} shadow-sm space-y-2`}>
        <div className="flex items-start gap-3">
          <div className={`w-10 h-10 rounded-lg shrink-0 flex items-center justify-center shadow-sm ${tierConfig.badgeBg}`}>
            <ActionIcon className="w-5 h-5" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-white/90 border border-border-soft text-text-main shadow-sm">
            ...
            <h3 className="text-base sm:text-lg font-bold mt-1 text-text-main leading-snug font-display">
              {tierConfig.actionTitle}
            </h3>
            <p className="text-xs sm:text-sm text-text-muted mt-1 leading-relaxed font-medium">
